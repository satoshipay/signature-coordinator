...
CREATE TABLE signature_request_cosigners (
  signature_request UUID REFERENCES signature_requests(id) ON DELETE CASCADE,
  cosigner_account_id VARCHAR(56) NOT NULL,
  has_signed BOOLEAN NOT NULL,
  PRIMARY KEY (signature_request, cosigner_account_id)
);

CREATE INDEX ON signature_request_cosigners(cosigner_account_id);
CREATE INDEX ON signature_request_cosigners(has_signed);
